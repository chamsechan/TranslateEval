...
import type { EvaluatorJob, ThresholdSummary } from '../types'
...
function ResultDetail({ jobId }: { jobId: string }) {
  ...
  return <>
    ...
    {context.evaluator.status !== 'completed' && !context.evaluator.error && <Alert type="warning" showIcon message={active.has(context.evaluator.status) ? '该任务尚未完全结束' : '任务已结束，存在失败或取消项'} action={context.evaluator.failed_items > 0 ? <Button loading={retrying} disabled={active.has(context.evaluator.status)} onClick={retry}>重试失败项</Button> : undefined} description="以下统计仅基于当前成功评分的样本，并始终显示覆盖率。" style={{ marginBottom: 16 }} />}
    ...
    <Card className="result-hero" style={{ marginBottom: 18 }}>
      {summary ? <Row align="middle" gutter={28}>
        ...
        <Col flex="auto"><Row gutter={26}>
          ...
          <Col span={6}><Statistic title="宏准确率" value={summary.macro_accuracy == null ? '—' : summary.macro_accuracy * 100} precision={1} suffix="%" /></Col>
    ...
    <Row gutter={[18, 18]}>
      ...
      <Col xs={24} xl={7}><Card className="panel-card" title="动态准确阈值" style={{ height: '100%' }}>
        ...
        <div className="threshold-control"><Slider min={0} max={max} step={0.1} value={draftThreshold} onChange={setDraftThreshold} onChangeComplete={setThreshold} style={{ flex: 1, minWidth: 0 }} /><InputNumber min={0} max={max} step={0.1} value={draftThreshold} onChange={(value) => { const next = value ?? 0; setDraftThreshold(next); setThreshold(next) }} /></div>
    ...
    {summary && <Card className="panel-card" title="逐语种覆盖与失败情况" style={{ marginTop: 18 }}>
      <Table rowKey="source_language" size="small" pagination={false} scroll={{ x: 600 }} dataSource={summary.by_language} columns={[
        { title: '语种', dataIndex: 'source_language' },
        { title: '成功 / 总数', render: (_, row) => `${row.count} / ${row.total}` },
        { title: '覆盖率', render: (_, row) => `${(row.coverage * 100).toFixed(1)}%` },
        { title: '平均分', dataIndex: 'mean', render: (value: number | null) => formatScore(value) },
        { title: '阈值通过率', render: (_, row) => row.accuracy == null ? '—' : `${(row.accuracy * 100).toFixed(1)}%` },
    ...
    <Card className="panel-card" title="逐句原始结果" style={{ marginTop: 18 }} extra={<Space><Select value={language} allowClear placeholder="语种" style={{ width: 110 }} options={summary?.by_language.map((item) => ({ label: item.source_language, value: item.source_language }))} onChange={(value) => { setLanguage(value); setPage(1) }} /><Select value={status} allowClear placeholder="状态" style={{ width: 120 }} options={[{ label: '成功', value: 'completed' }, { label: '失败', value: 'failed' }, { label: '已取消', value: 'cancelled' }]} onChange={(value) => { setStatus(value); setPage(1) }} /></Space>}>
      ...
      <Table scroll={{ x: 800 }} loading={itemsQuery.loading} onChange={(_, __, sorter, extra) => {
        ...
        { title: '分数', dataIndex: 'score', width: 100, sorter: true, sortOrder, render: (value: number | null, row) => <span className="score-cell">{formatScore(value)} {row.unit}</span> },
