  ...
  return <>
    ...
    <Card className="result-hero" style={{ marginBottom: 18 }}>
      <div className="result-threshold-toolbar">
        <Space wrap><strong>正确判定：得分 ≥</strong><InputNumber aria-label="准确阈值" min={0} max={max} step={0.1} value={threshold} onChange={value => updateFilters({ threshold: value })} /><span>（0–{max} {context.evaluator.evaluator_type === 'sacrebleu_zh' ? 'BLEU' : '分'}）</span></Space>
    ...
    <Collapse className="panel-card" style={{ marginTop: 18 }} items={[{ key: 'charts', label: <Space><LineChartOutlined />语种图表与识别统计</Space>, children: <>
      ...
      {languageDetection && <Typography.Paragraph type="secondary">{languageDetection.applicable ? `自动语种识别：准确率 ${languageDetection.accuracy == null ? '—' : `${(languageDetection.accuracy * 100).toFixed(2)}%`}，覆盖率 ${((languageDetection.coverage ?? 0) * 100).toFixed(2)}%` : `语种识别不适用：${languageDetection.reason}`}</Typography.Paragraph>}
    </> }]} />
