...
import { Alert, App, Button, Card, Empty, Input, InputNumber, Modal, Select, Space, Table, Tabs, Tag, Typography } from 'antd'
...
export default function ResultsListPage() {
  ...
  const openResult = (jobId: string, unscored = false, threshold?: number, language?: string) => {
  ...
  return <>
    <PageHeader title="评测结果" subtitle="直接查看每次评测的加权通过率，进入分析结果核对各语言对和逐句评分。" actions={<Space wrap><InputNumber aria-label="对比阈值" min={0} max={100} step={0.1} value={compareThreshold} onChange={(value) => setCompareThreshold(value ?? 0)} addonBefore="对比阈值" style={{ width: 200 }} /><Button type="primary" title="选择 2–12 条结果进行比较" loading={comparing} disabled={selectedJobs.length < 2 || selectedJobs.length > 12} onClick={compare}>对比所选 ({selectedJobs.length})</Button></Space>} />
    ...
    <Card className="panel-card">
      ...
      <Typography.Paragraph type="secondary" style={{ marginBottom: 14 }}>共 {data?.total ?? '—'} 条评测记录。通过率 = 通过数 / 总样本数，未评分计未通过；每行使用该评价修订的默认阈值，平均分仅统计已评分样本。进行中的结果会继续更新。点击列标题：升序 → 降序 → 默认。</Typography.Paragraph>
    ...
    <Modal title={`模型结果对比 · 阈值 ≥ ${comparison?.threshold ?? compareThreshold}`} width={1180} open={compareOpen} footer={null} onCancel={() => setCompareOpen(false)}>
      <Typography.Paragraph type="secondary">通过率按总样本数计算，未评分计未通过。加权平均通过率 = 总通过数 / 总样本数；宏平均通过率按语种等权。平均分仅基于已评分样本。下表统一使用对比阈值，可能与列表中的默认阈值不同。点击列标题：升序 → 降序 → 默认。</Typography.Paragraph>
      ...
      <Tabs defaultActiveKey="languages" items={[{ key: 'languages', label: '按语种对比', children: comparison && <ComparisonMatrix key={comparison.items.map(item => item.evaluator_job_id).join(',')} items={comparison.items} comparable={comparison.strictly_comparable} onOpen={(jobId, language) => openResult(jobId, false, comparison.threshold, language)} /> }, { key: 'overall', label: '总体指标', children: <Table<CompareRow> rowKey="evaluator_job_id" size="small" scroll={{ x: 1330, y: '55vh' }} pagination={false} dataSource={comparison?.items || []} columns={sortableColumns<CompareRow>([
        ...
        { key: 'micro_accuracy', title: '加权平均通过率', width: 158, render: (_, row) => <div><Typography.Text strong>{percentage(row.micro_accuracy)}</Typography.Text><div>通过 {row.passed} / 总数 {row.total}</div></div> },
        ...
        { key: 'macro_accuracy', title: '宏平均通过率', width: 130, dataIndex: 'macro_accuracy', render: (value: number | null) => percentage(value) },
