...
export default function DatasetSamples({ version, onClose, initialLanguage, initialSampleId, initialPage = 1, onFiltersChange }: DatasetSamplesProps) {
  ...
  return <Modal title={`样本预览 · ${version.dataset_name || version.dataset_key} · ${version.version_label}`} width={1100} open footer={null} onCancel={onClose}>
    ...
    <Typography.Paragraph type="secondary">当前匹配 {query.data?.total ?? '—'} 条。样本 ID 按完整内容精确匹配，可与评分明细中的样本 ID 核对。</Typography.Paragraph>
    <QueryError error={query.error} retry={query.refresh} />
    <Table rowKey="sample_id" loading={query.loading} dataSource={query.data?.items || []} size="small" scroll={{ x: 1040 }} locale={{ emptyText: language || sampleId ? '没有符合当前筛选条件的样本，请调整或清除筛选。' : '此版本暂无样本' }} pagination={{ current: page, pageSize: 20, total: query.data?.total || 0, showSizeChanger: false, showTotal: total => `共 ${total} 条`, onChange: nextPage => changeFilters({ language, sampleId, page: nextPage }) }} columns={[
      { title: '样本 ID', dataIndex: 'sample_id', width: 160, render: (value: string) => <Typography.Text copyable style={{ overflowWrap: 'anywhere' }}>{value}</Typography.Text> },
      { title: '语言对', dataIndex: 'source_language', width: 220, render: (value: string) => languagePairLabel(value, pairs.find(pair => pair.source_language === value)?.source_name) },
      { title: '源文', dataIndex: 'source_text', width: 330, render: (value: string) => <div style={{ whiteSpace: 'pre-wrap', overflowWrap: 'anywhere' }}>{value}</div> },
      { title: '中文参考译文', dataIndex: 'reference_zh', width: 330, render: (value: string) => <div style={{ whiteSpace: 'pre-wrap', overflowWrap: 'anywhere' }}>{value}</div> },
    ]} />
  </Modal>
}
